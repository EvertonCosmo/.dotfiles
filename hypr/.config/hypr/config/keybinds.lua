...
local lock = "swaylock-fancy -e -K -p 10 -f Hack-Regular"

-- See https://wiki.hypr.land/Configuring/Basics/Binds/

-- Laptop monitor off / on
-- Note: `hyprctl keyword` is rejected under the Lua config parser, so call hl.monitor directly.
hl.bind(mainMod .. " + F10", function()
	hl.monitor({ output = "eDP-1", disabled = true })
end)
hl.bind(mainMod .. " + F11", function()
	hl.monitor({ output = "eDP-1", mode = "highres", position = "auto", scale = 1, disabled = false })
end)
...
-- Switch workspaces with mainMod + key
